...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>诗词主题：</div>
        <el-input v-model="formSearch.shicizhuti" placeholder="诗词主题" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>诗词名称：</div>
        <el-input v-model="formSearch.shicimingcheng" placeholder="诗词名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>诗词作者：</div>
        <el-input v-model="formSearch.shicizuozhe" placeholder="诗词作者" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('shicijianshang','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"10px","background":"#FFDFA6","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shicijianshangAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"padding":"20px 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"1px solid #5FE3EB","width":"200px","padding":"10px","overflow":"hidden","borderRadius":"10px","height":"250px"}' class="img-box">
				<img @click.stop="imgPreView(item.shicitupian)" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-if="item.shicitupian && item.shicitupian.substr(0,4)=='http'" :src="item.shicitupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.shicitupian?item.shicitupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-else :src="baseUrl + (item.shicitupian?item.shicitupian.split(',')[0]:'')" class="image" />
			</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/shicichaodai/shicichaodai').then(res => {
			if (res.data.code == 0) {
				this.shicichaodaiOptions = res.data.data;
				this.selectOptionsList.push({name:'诗词朝代',list:this.shicichaodaiOptions,tableName: 'shicichaodai',check: -1})
			}
		});
		await this.$http.get('option/shicifenlei/shicifenlei').then(res => {
			if (res.data.code == 0) {
				this.shicifenleiOptions = res.data.data;
				this.selectOptionsList.push({name:'诗词分类',list:this.shicifenleiOptions,tableName: 'shicifenlei',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.shicizhuti = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.shicizhuti != '') searchWhere.shicizhuti = '%' + this.formSearch.shicizhuti + '%';
        if (this.formSearch.shicimingcheng != '') searchWhere.shicimingcheng = '%' + this.formSearch.shicimingcheng + '%';
        if (this.formSearch.shicichaodai != '') searchWhere.shicichaodai = this.formSearch.shicichaodai;
        if (this.formSearch.shicifenlei != '') searchWhere.shicifenlei = this.formSearch.shicifenlei;
        if (this.formSearch.shicizuozhe != '') searchWhere.shicizuozhe = '%' + this.formSearch.shicizuozhe + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`shicijianshang/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/shicijianshangDetail', query: params});
      },
