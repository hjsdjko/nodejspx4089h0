<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="诗词分类" prop="shicifenlei">
					<el-input v-model="ruleForm.shicifenlei" placeholder="诗词分类" clearable  :readonly="ro.shicifenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="诗词分类" prop="shicifenlei">
					<el-input v-model="ruleForm.shicifenlei" placeholder="诗词分类" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		return {
			...
			rules: {
				shicifenlei: [
					{ required: true, message: '诗词分类不能为空', trigger: 'blur' },
				],
			},
		};
	},
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shicifenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shicifenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
