<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="诗词朝代" prop="shicichaodai">
					<el-input v-model="ruleForm.shicichaodai" placeholder="诗词朝代" clearable  :readonly="ro.shicichaodai"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="诗词朝代" prop="shicichaodai">
					<el-input v-model="ruleForm.shicichaodai" placeholder="诗词朝代" readonly></el-input>
				</el-form-item>
			</template>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shicichaodai/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shicichaodaiCrossAddOrUpdateFlag = false;
											this.parent.search();
