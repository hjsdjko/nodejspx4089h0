...
import HuodongfenleiModel from '../models/HuodongfenleiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let huodongfenlei = req.query.huodongfenlei
			if (huodongfenlei) {

				if (huodongfenlei.indexOf('%') != -1) {
					where.huodongfenlei = {
						[Op.like]: huodongfenlei
					}
				} else {
					where.huodongfenlei = {
						[Op.eq]: huodongfenlei
			...
			let result = await HuodongfenleiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await HuodongfenleiModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await HuodongfenleiModel.findOne({where:dictionary})
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			let huodongfenleicount = await HuodongfenleiModel.count({
				where: {
					huodongfenlei: req.body["huodongfenlei"]
				}
			})
            if (huodongfenleicount > 0) {
				toRes.session(res, -1, '活动分类已存在')
            }



			const userinfo = await HuodongfenleiModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await HuodongfenleiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete',(req,res,next)=>{
		...
		try {
			...
			await HuodongfenleiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await HuodongfenleiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM huodongfenlei WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "huodongfenlei"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM huodongfenlei " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("huodongfenlei" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM huodongfenlei " + where + " GROUP BY " + xColumnName + " DESC"
